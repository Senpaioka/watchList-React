            ...
            <div style={starDivStyle}>
                {
                    Array.from({length: maxRating}, (_, i) => (
                        <Star key={i} onRate={() => handleRating(i + 1)} full={tempRating ? tempRating >= i+1 : rating >= i+1} onHoverIn={() => setTempRating(i+1)} onHoverLeave={() => setTempRating(0)} color={color} size={size}></Star>
                    ))
                }
            </div>
...
function Star({onRate, full, onHoverIn, onHoverLeave, color, size}) {
    ...
    return(
    <span role="button" style={starStyle} onClick={onRate} onMouseEnter={onHoverIn} onMouseLeave={onHoverLeave}>
        {
            full ? (
                <svg
